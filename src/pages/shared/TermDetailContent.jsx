...
function TermDetailContent() {
  ...
  const { masteredIds, toggleMastery } = useMastery()
      ...
      {!loading && term && (
        <article className="card term-detail-card">
          ...
          <h1 className="term-detail-title">
            ...
            <button
              type="button"
              className={`quote-icon-btn term-mastery-btn${masteredIds.has(term.id) ? ' active' : ''}`}
              onClick={() => toggleMastery(term)}
              aria-label={
                masteredIds.has(term.id)
                  ? t.account.home.mastery.unmarkLearned
                  : t.account.home.mastery.markLearned
              }
              title={
                masteredIds.has(term.id)
                  ? t.account.home.mastery.unmarkLearned
              ...
              aria-pressed={masteredIds.has(term.id)}
            >
...
export default TermDetailContent
